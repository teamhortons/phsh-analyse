...
module.exports = {
	...
	init_label_accuracies: function() {
		...
		for (var c = 0; c < alllabels.length; c++) {
			var cur = alllabels[c];
			this.label_accuracies[c] = {
				label: cur,
				window: new cnnutil.Window(100, 1)
			};
			if (cur === "negative") {
	...
	get_all_label_configs: function() {
		...
		for (let label of this.get_all_labels()) {
			var json = JSON.parse(
				fs.readFileSync(DATA_ROOT + label + "/config.json")
			);
			ret.push(json);
			console.log(json);
		}
		...
		for (var c = 0; c < alllabels.length; c++) {
			...
			for (var f = 0; f < files_for_label.length; f++) {
				...
				allsamples.push({
					label: c,
					label_str: cur,
					data: this.create_volume_for_image_path(filepath)
				});
			}
		}
		return allsamples;
	...
	evaluate_activation: function(activations, labels) {
		var preds = [];
		for (var k = 0; k < activations.w.length; k++) {
			preds.push({ k: k, p: activations.w[k] });
		}
